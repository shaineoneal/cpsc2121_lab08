...
Node *insert(Node *T, int v, int r)
{
  ...
  int rootRank = 0;
  //if tree doesn't exist, create the first node and return it
  if(T == nullptr) return new Node(v);

  //calculate rank of root node
  if(T->left != nullptr) rootRank = T->left->size;
